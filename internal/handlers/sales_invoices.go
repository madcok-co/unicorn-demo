...
func ListSalesInvoices(ctx *context.Context, req domain.ListSalesInvoicesDTO) (*pagination.OffsetResult, error) {
	db := getDB(ctx)
	...
	query := db.Model(&domain.SalesInvoice{}).
		Preload("Items").
	...
	if req.Search != "" {
		search := "%" + req.Search + "%"
		query = query.Where("code LIKE ? OR customer_name LIKE ? OR reference_no LIKE ? OR tax_invoice_no LIKE ?", search, search, search, search)
	}
	if req.CustomerID != "" {
	...
	if err := query.Offset(offset).Limit(req.Limit).Order("invoice_date DESC, created_at DESC").Find(&invoices).Error; err != nil {
		return nil, fmt.Errorf("failed to fetch sales invoices: %w", err)
	}
...
func GetSalesInvoice(ctx *context.Context, id string) (*domain.SalesInvoice, error) {
	db := getDB(ctx)
	tenantID, err := getTenantID(ctx)
	if err != nil {
		return nil, err
	...
	if err := db.Preload("Items").
		...
		return nil, fmt.Errorf("sales invoice not found: %w", err)
	}
...
func CreateSalesInvoice(ctx *context.Context, req domain.CreateSalesInvoiceDTO) (*domain.SalesInvoice, error) {
	db := getDB(ctx)
	tenantID, err := getTenantID(ctx)
	if err != nil {
		return nil, err
	...
	var existing domain.SalesInvoice
	if err := db.Where("code = ? AND tenant_id = ?", req.Code, tenantID).First(&existing).Error; err == nil {
		return nil, fmt.Errorf("sales invoice with code %s already exists", req.Code)
	}
	...
	// Get delivery order info if provided
	var deliveryOrderCode *string
	if req.DeliveryOrderID != nil {
		var deliveryOrder domain.DeliveryOrder
		if err := db.Where("id = ? AND tenant_id = ?", *req.DeliveryOrderID, tenantID).First(&deliveryOrder).Error; err == nil {
	...
	items := make([]domain.SalesInvoiceItem, len(req.Items))
	...
	for i, itemReq := range req.Items {
		...
		items[i] = domain.SalesInvoiceItem{
			ID:           uuid.New().String(),
			TenantID:     tenantID,
	...
	// If we have order item updates, create invoice in a transaction
	if len(orderItemUpdates) > 0 && order != nil {
		...
		if err := tx.Create(invoice).Error; err != nil {
			tx.Rollback()
			return nil, fmt.Errorf("failed to create sales invoice: %w", err)
		}
		...
		// Update sales order invoiced quantity
		...
		if err := tx.Model(&domain.SalesOrder{}).
			...
			tx.Rollback()
			return nil, fmt.Errorf("failed to update sales order invoiced quantity: %w", err)
		}
...
func UpdateSalesInvoice(ctx *context.Context, id string, req domain.UpdateSalesInvoiceDTO) (*domain.SalesInvoice, error) {
	db := getDB(ctx)
	tenantID, err := getTenantID(ctx)
	if err != nil {
		return nil, err
	...
	if req.CustomerPO != "" {
		invoice.CustomerPO = req.CustomerPO
	}
	if req.TaxInvoiceNo != "" {
	...
	if req.IncomeAccountID != "" {
		invoice.IncomeAccountID = req.IncomeAccountID
	}
	if req.ARAccountID != "" {
		invoice.ARAccountID = req.ARAccountID
	}
	...
	if req.BillingZip != "" {
		invoice.BillingZip = req.BillingZip
	}
	...
	if err := db.Save(&invoice).Error; err != nil {
		return nil, fmt.Errorf("failed to update sales invoice: %w", err)
	}
...
func DeleteSalesInvoice(ctx *context.Context, id string) error {
	db := getDB(ctx)
	tenantID, err := getTenantID(ctx)
	if err != nil {
		return err
	...
	var invoice domain.SalesInvoice
	if err := db.Preload("Items").Where("id = ? AND tenant_id = ?", id, tenantID).First(&invoice).Error; err != nil {
		return fmt.Errorf("sales invoice not found: %w", err)
	}
	...
	// If invoice is linked to an order, revert the invoiced quantities
	if invoice.OrderID != nil {
		...
		if err := tx.Preload("Items").Where("id = ? AND tenant_id = ?", *invoice.OrderID, tenantID).First(&order).Error; err == nil {
			...
			// Recalculate sales order invoiced quantity
			...
			if err := tx.Model(&domain.SalesOrder{}).
				...
				tx.Rollback()
				return fmt.Errorf("failed to update sales order invoiced quantity: %w", err)
			}
		}
		...
		if err := tx.Save(&invoice).Error; err != nil {
			tx.Rollback()
			return fmt.Errorf("failed to delete sales invoice: %w", err)
		}
...
// GetSalesInvoiceStats returns sales invoice statistics
func GetSalesInvoiceStats(ctx *context.Context) (map[string]interface{}, error) {
	db := getDB(ctx)
	tenantID, err := getTenantID(ctx)
	if err != nil {
		return nil, err
	...
	db.Model(&domain.SalesInvoice{}).Where("tenant_id = ? AND active = ?", tenantID, true).Count(&totalInvoices)
